...
>>>>>>> 164c717 (Fix dataframe drop to ignore missing 'id' column)
...
with st.expander("Find / pick a vendor to edit", expanded=True):
    ...
    st.dataframe(df.drop(columns=["id"], errors="ignore"), use_container_width=True, hide_index=True)df = pd.DataFrame(rows, columns=["id", "Business", "Category", "Phone", "Address", "Website", "Active"])
